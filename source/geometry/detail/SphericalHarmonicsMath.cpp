...
namespace detail
{
    ...
    double K(int l, int m)
    {
        ...
        double temp =
            ((2.0 * l + 1.0) * static_cast<double>(factorial(l - m))) /
    ...
    double SH(int l, int m, double theta, double phi)
    {
        if (m == 0)
            ...
            return sqrt2 * K(l, m) * std::cos(m * phi) *
                   P(l, m, std::cos(theta));
        else
            return sqrt2 * K(l, -m) * std::sin(-m * phi) *
                   P(l, -m, std::cos(theta));
    }
               ...
               double &dphi)
    {
        ...
        else if (m > 0)
        {
            ...
            dtheta =
                sqrt2 * Klm * cosm * dPlm_mu(l, m, u, Plm) * (-std::sin(theta));
        }
        else // m < 0
        {
            double u = std::cos(theta);
