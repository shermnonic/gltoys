...
///////////////////////////////////////////////////////////////////////////////
//! basic vector type
template <typename SCALAR, unsigned int DIM, typename VECTOR> class Vector
{
  protected:
    SCALAR m_v[DIM];
  ...
  private:
    ...
    inline const VECTOR &as_child() const
    {
  ...
  public:
    ...
    inline VECTOR operator*(const SCALAR &s) const
    {
        return MATH_CLONE_VECTOR *= s;
    }
    inline VECTOR operator/(const SCALAR &s) const
    {
        return MATH_CLONE_VECTOR /= s;
...
{
    //	friend Vector4<SCALAR> operator * ( const Matrix4x4&, const
    // Vector4<SCALAR>& );
...
class Vector3 : public Vector<SCALAR, 3, Vector3<SCALAR>>
{
  public:
    ...
    inline Vector3<SCALAR> cross(const Vector3<SCALAR> w) const
    {
        ...
        //				         x*w.y - y*w.x );
        // return *this;
        return Vector3<SCALAR>(y * w.z - z * w.y, z * w.x - x * w.z,
                               x * w.y - y * w.x);
    };
